...
const Navbar = () => {
  ...
  const [ssubLinks, setSsubLinks] = useState([]); // Ensure ssubLinks is initialized as an empty array
  const [loading, setLoading] = useState(false);
  ...
  const fetchSsubLinks = async () => {
    ...
    try {
      const result = await apiConnector("GET", categories.CATEGORIES_API);
      setSsubLinks(result?.data?.data || []); // Ensure data is an array or default to empty array
      console.log(result);
    } catch (error) {
      console.log("Cannot fetch the category list");
      setSsubLinks([]); // Fallback to an empty array on error
    }
    setLoading(false);
  };
                  ...
                  {link.title === "Catalog" ? (
                    ...
                    >
                      ...
                      <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]">
                        ...
                        {loading ? (
                          <p className="text-center">Loading...</p>
                        ) : ssubLinks.length > 0 ? ( // Ensure ssubLinks has length before mapping
                          ssubLinks
                            ?.filter((subLink) => subLink?.courses?.length > 0) // Ensure courses is valid before checking length
                            ?.map((sublink, index) => (
                              <Link
                                className="hover:bg-richblack-50 p-2 rounded-md font-medium hover:font-semibold"
                                to={`/catalog/${sublink.name
        ...
        <>
          ...
          >
            <div className="w-11/12 max-w-maxContent items-center my-4">
              ...
              <div className="flex w-full flex-col gap-y-4 items-center justify-center mt-4 md:hidden">
                {user && user?.accountType !== ACCOUNT_TYPE.INSTRUCTOR && (
                  <Link to="/dashboard/cart" className="relative">
                    <AiOutlineShoppingCart className="text-2xl text-richblack-100" />
                    {totalItems > 0 && (
